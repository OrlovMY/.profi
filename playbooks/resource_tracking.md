...
Запись в `memory/project_resource_tracking.md` — **тем же ходом, каким принимается финальный отчёт deploy-агента**: отчёт, фиксирующий heavy resource usage, не считается принятым, пока строка не легла в журнал. Инцидент без выката — тот же порядок: запись разбора инцидента с heavy resource usage (`scenarios/post-incident-memory-update.md`, триггер 6) не считается принятой без строки в журнале. Сбор чисел уже обязателен в самом отчёте (раздел «Что обязательно собрать…» ниже); перенос — часть приёмки, а не отдельный шаг «потом»: отдельный ручной шаг перестаёт делаться ровно тогда, когда работы много (`knowledge/standards/attach-recurring-step-to-mandatory-action.md`). Признак несрабатывания: дата изменения журнала старше даты последнего деплоя с heavy usage (`rule-expiry-and-ownership.md`, п. 2). В проекте, где журнал не заведён или погиб, — сначала завести файл по `templates/resource_log.md`: на 19.08.2026 журнал проекта-источника был 84 дня без записей, 22.08 погиб вместе с памятью, в реконструкцию 24.08 не вошёл — и четыре места свода продолжали требовать запись в несуществующий файл. Также включать в финальные отчёты агентов вопрос «какие ресурсы это потребляло» (см. `templates/agent_briefing.md`).
...
История правок: `archive/2026-08/ревизия-свода-2026-08-24.md`, П7 (в редакции PQ-01 — журнал ресурсов привязан к приёмке отчёта deploy-агента И записи разбора инцидента; возврат в работу по `attach-recurring-step-to-mandatory-action.md`); вердикты — `archive/2026-08/PQ-01-ревизия-2026-08-24.md`; 2026-08-24.
