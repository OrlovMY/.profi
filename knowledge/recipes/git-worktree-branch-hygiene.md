...
2. **Merged-ветки — безопасно:** `git branch --merged <main> | grep -vE '^\* |^ *<main>$' | xargs -r git branch -d` (`-d`, не `-D` — откажется на несмерженном). Подставить имя основной ветки (`main`/`master`). Фильтр заякорен: неякорный `<main>` выбрасывал любую ветку с подстрокой main/master в имени (`maintenance-fix`, `domain-cleanup`) — тихо и без ошибки.
