...
2. **Merged-ветки — безопасно:** `git branch --merged <main> | grep -vE '^\* |^ *<main>$' | xargs -r git branch -d` (`-d`, не `-D` — откажется на несмерженном). Подставить имя основной ветки (`main`/`master`). Фильтр заякорен: неякорный `<main>` выбрасывал любую ветку с подстрокой main/master в имени (`maintenance-fix`, `domain-cleanup`) — тихо и без ошибки. Выполнять стоя на `<main>` (или предварить `git checkout <main>`): фильтр `--merged` сверяет с `<main>`, а `git branch -d` следом — с ТЕКУЩИМ HEAD; с другой ветки влитые получают ложное «not fully merged», толкающее к `-D` (случай — `git-branch-discipline.md`, выноска при аудите, 25.08.2026).
...
История правок: `archive/2026-08/ревизия-свода-2026-08-24.md`, раздел 13, Б9 (фильтр заякорен — неякорный терял ветки с подстрокой main/master; подсадка в разборе); вердикты — `archive/2026-08/PQ-01-ревизия-2026-08-24.md`, «Дополнение по разделу 13»; 2026-08-25.

История правок: `archive/2026-08/PE-01-разбор-2026-08-25.md`, раздел 8 (Д-Р2); вердикты — `archive/2026-08/PQ-01-ревизия-2026-08-24.md`, «Разбор дополнения 25.08 (поздний вечер) — Д-Р1, Д-Р2»; 2026-08-25.
